...
class LSTMCell: 
    ...
    def forecast1(self, forecastingData):
        self.forwardPass(forecastingData,"no_prediksi")
        f_h = np.transpose(np.transpose(self.h[-1]))
        return (f_h)
...
def myprediksi(forecastSequences,lstm,max_ex,min_ex,sequenceLength):
    ...
    # diambil squence yang paling bawah
    data = data_myprediksi[-1].tolist()
    print (data,"squen paling terakhir")
    ...
    e = []
    datelist = pd.bdate_range(waktu, periods=sequenceLength+1)
    ...
    for i in range(sequenceLength):
        ...
        forecasty  = lstm.forecast(data[-5:])
        V_Predict_nor = forecasty.tolist()
        waktuu = float(e[i+1])
        waktuu -= min(e)
        waktuu /= max(e)
        V_Predict_denor = forecasty
        V_Predict_denor *= max_ex[1:]
        V_Predict_denor += min_ex[1:]
        save_prediksi.append(V_Predict_denor)
        data.append([1, waktuu, V_Predict_nor[0]])
        forecasts.append(data[-1])
