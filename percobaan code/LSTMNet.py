...
def readData(filename):
    ...
    for line in lines:
        if i < 100000 and '?' not in line:
            ...
            example = [month, time, global_active_power]
...
def main():
    ...
    data = readData('satu-tujuh_fitur.txt')
    corpusData = data[0]
    max_ex = data[1]
    min_ex = data[2]
    trainingData = corpusData[:-50]
    forecastData = corpusData[50:100]
    ...
    forecastData *= max_ex
    forecastData += min_ex
    # print ("--- forecastData[ ",forecastData[forecastData.shape[0] - forecasts.shape[0]:,2])
